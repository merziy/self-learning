  ...
  App.findById(req.params.messageId)
    ...
    .catch((err) => {
      ...
      return res.status(500).send({
        message: "Error retrieving message with id " + req.parans.messageId,
      });
    });
};
...
// Delete message with the specified messageId in the request
exports.delete = (req, res) => {
  App.findByIdAndRemove(req.params.messageId)
    ...
    .catch((err) => {
      if (err.kinf === "ObjectId" || err.name === "NotFound") {
        return res.status(404).send({
          message: "Message not found with id " + req.params.messageId,
        });
      }
